...
The purpose of this hook is to verify that the branch to be uploaded contains the latest changes from the develop branch. So before pushing to the repository, the hook will download the latest changes from the develop branch to its local repository and perform a merge.

First you have to run the following command to tell git that when it pulls it will also merge and not rebase
...
```bash
aws cloudformation delete-stack --stack-name creator-space
```
