...
```bash
event go$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
event go$ sam logs -n HelloWorldFunction --stack-name event go --tail
```
...
```bash
sam delete --stack-name event go
```
