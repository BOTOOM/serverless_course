...
```bash
EventBrige-Rule$ sam build --use-container
```
...
```bash
EventBrige-Rule$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
EventBrige-Rule$ sam logs -n HelloWorldFunction --stack-name EventBrige-Rule --tail
```
...
```bash
EventBrige-Rule$ pip install -r tests/requirements.txt --user
# unit test
EventBrige-Rule$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
EventBrige-Rule$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name EventBrige-Rule
```
